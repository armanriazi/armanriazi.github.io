...
> _Smart Contracts, MultiSignature, RPC, Make A Better CLI, and something that you can implementing_ (Do not worry sice most of your works will merge to main branch. We will not create a framework or complete Blockchain because we just need to **learning** more and used it use-cases)
...
_Using time and watch is optional and depends on your purpose_
...
> Similar to how you use the Deref trait to override the _operator on immutable references, you can use the DerefMut trait to override the_ operator on mutable references.the Drop trait is almost always used when implementing a smart pointer. For example, when a Box<T> is dropped it will deallocate the space on the heap that the box points to.
