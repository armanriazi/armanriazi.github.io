
# Video Toturials Self-Study

> I prefer to find important chapters that related to my work specially cybersecurity. I gathered significant INFOs on my desktop(offline documentation)

!!! Learnt Toturials

    === "Cybersecurity Books Epub On Desktop 2025"               
        * Gray Hat Hacking The Ethical Hacker’s Handbook, Third Edition, Allen Harper, Shon Harris, Jonathan Ness, Chris Eagle, Gideon Lenkey, and Terron Williams.
        * Information Security Management An Executive View Marcos Sêmola Translation from original version by Ricardo Bruno Beckman Soares da Cruz.        
        * MASTERING CYBERSECURITY MEASUREMENT, axonius
        * Intrusion Detection Systems with Snort Advanced IDS Techniques Using Snort, Apache, MySQL, PHP, and ACID.
        * Security Engineering, Ross Anderson, Cambridge.
        * Crypto 101, Laurens Van Houtven
        * OWASP
        * Black Hat Rust_ Applied offensive security with the Rust programming language.

    === "Video courses 2019-2024"

        * There are some integration on tools and technologies, so it's a part of cybersecurity toturials. [Cybersecurity-Toturials](../../cybersecurity/learning/cybersecurity-non-cert-toturials.md)        

    === "Video courses 2018-2019"

        * Below list got from my external HD(archived)
        * After all of them I just think I dunno anymore
        * I still have way to go
       
---

#### Followed Youtube Channels

* Everything about Tools in Cybersecurity
* I do not remember, there are so many playlists


#### Resources

- [Learning Cybersecurity](../../../assets/attachments/learning-cysec1.jpg)